...
const RightNextButton: React.FC<ButtonProps> = ({ className, children, onClick }) => {
    return (
        <button
            className={` ${className} hover:bg-[#eeecec] border-none absolute w-12 h-12 rounded-full bg-[#fff] text-[#fff] flex items-center justify-center top-[calc(50%-1.5rem)] cursor-pointer right-2 transition-colors duration-200 ease-in`}
            onClick={onClick}
        >
