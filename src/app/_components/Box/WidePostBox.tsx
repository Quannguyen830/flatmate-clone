...
const WidePostBox: React.FC<ImageProps> = ({ className, children, imagesList, imageLength, link }) => {
    return (
        <div className={`shadow-[0_0_8px_rgba(0,0,0,0.16)] mx-auto mb-8 max-w-full w-full rounded-[12px] ${className}`}>
            <WidePostImageContainer imagesList={imagesList} imageLength={imageLength}>

            </WidePostImageContainer>

            <a href={`/${link}`} className="px-6 pt-4 pb-6 h-full bg-white block">
                <div className="p-0">
                    {children}
                </div>
            </a>
